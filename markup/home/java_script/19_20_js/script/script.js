...
var obj = _.map(arr, 'skills'),
    skills = _.flattenDepth(obj, 2);
    skills = _.uniq(skills);
    skills = _.sortBy(skills, _.toLower);

    console.log('User skills: ', skills);
// -------------------------------------


  var userName = _.map(arr, function(o) {
    return { "name": o.name, "friends": o.friends};
  });

var nameOrder = _.map(_.sortBy(userName, 'friends.length'), 'name');
    nameOrder = _.reverse(nameOrder);

console.log('Name Order: ', nameOrder);
// --------------------------------------

var obj2 = _.map(arr, function(o) {
    return {"name": _.map(o.friends, "name")};
}),
   userFriends = _.map(obj2, 'name'),
   userFriends = _.flattenDepth(userFriends, 2);
   userFriends = _.uniq(userFriends);
